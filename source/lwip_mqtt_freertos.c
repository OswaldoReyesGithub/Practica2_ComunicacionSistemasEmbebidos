...
static void mqtt_subscribe_topics(mqtt_client_t *client)
{
    ...
    sprintf(topic, "OswaldoR/feeds/valvula-gas");
    err = mqtt_subscribe(client, topic, 0, mqtt_topic_subscribed_cb, LWIP_CONST_CAST(void *, "Valvula_Gas"));
    if (err == ERR_OK) PRINTF("Subscribing to the topic \"%s\" with QoS %d...\r\n", topic, 0);
    else PRINTF("Failed to subscribe to the topic \"%s\" with QoS %d: %d.\r\n", topic, 0, err);

    sprintf(topic, "OswaldoR/feeds/valvula-agua");
    err = mqtt_subscribe(client, topic, 0, mqtt_topic_subscribed_cb, LWIP_CONST_CAST(void *, "Valvula_Agua"));
    if (err == ERR_OK) PRINTF("Subscribing to the topic \"%s\" with QoS %d...\r\n", topic, 0);
    else PRINTF("Failed to subscribe to the topic \"%s\" with QoS %d: %d.\r\n", topic, 0, err);
}
 ...
 */
static void publish_s_gas(void *ctx)
{
	LWIP_UNUSED_ARG(ctx);
	...
	sprintf(topic, "OswaldoR/feeds/sensor");
	sprintf(message, "%d", s_gas);
...
static void publish_s_agua(void *ctx)
{
	...
	sprintf(topic, "OswaldoR/feeds/alarma-temp");
	sprintf(message, "%d", s_agua);
...
static void publish_liquido(void *ctx)
{
	...
	sprintf(topic, "OswaldoR/feeds/sensor-liquido");
...
static void publish_energia(void *ctx)
{
	...
	sprintf(topic, "OswaldoR/feeds/energia");
...
static void app_thread(void *arg)
{
    ...
    for (;;)
    {
        if (connected)
        {
        	s_gas = rand()%21;
        	PRINTF("NivelC02: %d \r\n", s_gas);
            err = tcpip_callback(publish_s_gas, NULL);
            if (err != ERR_OK)
            {
                PRINTF("Failed to invoke publishing of a NivelGas on the tcpip_thread: %d.\r\n", err);
            }

        	if (s_gas > 10)
        	{
        		s_agua = 1;
                err = tcpip_callback(publish_s_agua, NULL);
                if (err != ERR_OK)
                {
                    PRINTF("Failed to invoke publishing of a Nivel_Agua on the tcpip_thread: %d.\r\n", err);
                }
        		PRINTF("El Nivel se encuentra fuera del rango normal!! \r\n");
        		LED_BLUE_ON();
                sys_msleep(3000U);
                LED_BLUE_OFF();
                s_agua = 0;
        	...
        	if (liquido == 1)
			{
                ...
                if (err != ERR_OK)
                {
                    PRINTF("Failed to invoke publishing of a liquido change on the tcpip_thread: %d.\r\n", err);
                }
        		PRINTF("Se_ha derramado_liquido!! \r\n");
        		LED_RED_ON();
        		LED_BLUE_ON();
				LED_GREEN_ON();
				sys_msleep(3000U);
            ...
            PRINTF("Existe_Carga_De_Energia? %d \r\n", energia);
    		err = tcpip_callback(publish_energia, NULL);
    		if (err != ERR_OK)
    		{
            PRINTF("Failed to invoke publishing of a message about energia on the tcpip_thread: %d.\r\n", err);
    		}
